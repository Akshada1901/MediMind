...
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Cell, ResponsiveContainer, RadarChart, PolarGrid, PolarAngleAxis, Radar, PieChart, Pie, Legend } from "recharts";
import KnowledgeGraph from "./KnowledgeGraph";

const getRiskColor = (risk) =>
  risk === "High" ? "#ff4466" : risk === "Medium" ? "#ffb800" : "#00e5a0";

const card = { background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 20, padding: 28, marginBottom: 20 };
const secLabel = { fontSize: 11, fontWeight: 600, letterSpacing: "2px", textTransform: "uppercase", color: "rgba(232,237,248,0.4)", marginBottom: 16 };

export default function Dashboard({ results }) {
  const { entities, risks, summary, labs, analysis_time, anomalies, comparison } = results;
  ...
  const radarData = (risks || []).slice(0, 6).map(r => ({ subject: r.disease, value: Math.round(r.probability * 100) }));
  const riskCounts = { High: (risks||[]).filter(r=>r.risk==="High").length, Medium: (risks||[]).filter(r=>r.risk==="Medium").length, Low: (risks||[]).filter(r=>r.risk==="Low").length };
  const pieData = Object.entries(riskCounts).filter(([,v]) => v > 0).map(([name, value]) => ({ name, value, fill: getRiskColor(name) }));
    ...
    <div style={{ animation: "fadeUp 0.6s ease" }}>
      ...
      {anomalies && anomalies.length > 0 && (
        <div style={{ marginBottom: 24 }}>
          <div style={secLabel}>🚨 Clinical Anomaly Alerts</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {anomalies.map((alert, i) => (
              <div key={i} style={{ background: `${alert.color}12`, border: `1px solid ${alert.color}40`, borderLeft: `4px solid ${alert.color}`, borderRadius: 12, padding: "14px 20px", display: "flex", alignItems: "center", gap: 16 }}>
                <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: "1px", color: alert.color, background: `${alert.color}20`, padding: "3px 10px", borderRadius: 20, minWidth: 70, textAlign: "center" }}>
                  {alert.severity}
                </span>
                <span style={{ fontSize: 14, color: "rgba(232,237,248,0.85)", flex: 1 }}>{alert.message}</span>
                <span style={{ fontSize: 12, fontFamily: "'JetBrains Mono',monospace", color: alert.color, fontWeight: 600 }}>{alert.value}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* SUMMARY */}
      <div style={secLabel}>🤖 AI Clinical Summary</div>
      <div style={{ background: "linear-gradient(135deg,rgba(24,71,240,0.12),rgba(0,212,255,0.06))", border: "1px solid rgba(24,71,240,0.25)", borderRadius: 20, padding: "32px 36px", marginBottom: 28, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
        <div style={card}>
          <div style={secLabel}>🎯 Disease Risk Prediction</div>
          {(risks||[]).map((r) => {
        ...
        <div style={card}>
          <div style={secLabel}>🔬 Extracted Medical Entities</div>
          {Object.entries(entities||{}).filter(([k]) => !["RECOMMENDATIONS","CLINICAL_FINDINGS"].includes(k)).map(([label, values]) => (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 20, marginBottom: 20 }}>
        <div style={card}>
          <div style={secLabel}>📊 Risk Probability Distribution</div>
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={risks} margin={{ top: 10, right: 20, left: 0, bottom: 5 }}>
              ...
              <Tooltip contentStyle={{ background: "#0a1628", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 10, color: "#e8edf8", fontSize: 13 }} formatter={v=>[`${(v*100).toFixed(0)}%`,"Probability"]} />
              <Bar dataKey="probability" radius={[6,6,0,0]}>
                {(risks||[]).map((entry,i) => <Cell key={i} fill={getRiskColor(entry.risk)} fillOpacity={0.8} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
        <div style={card}>
          <div style={secLabel}>🥧 Risk Distribution</div>
          <ResponsiveContainer width="100%" height={260}>
            <PieChart>
              <Pie data={pieData} cx="50%" cy="45%" outerRadius={80} innerRadius={45} dataKey="value" paddingAngle={4}>
      ...
      {/* RADAR */}
      <div style={card}>
        <div style={secLabel}>🕸 Multi-Disease Risk Radar</div>
      ...
      {labs && Object.keys(labs).length > 0 && (
        <div style={card}>
          <div style={secLabel}>🧪 Extracted Lab Values</div>
      ...
      {comparison && (
        <div style={{ marginBottom: 20 }}>
          <div style={secLabel}>📈 Comparison with Previous Report ({comparison.previous_date})</div>
          <div style={card}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(180px,1fr))", gap: 12 }}>
              {comparison.risk_changes.map((rc) => (
                <div key={rc.disease} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: "14px 16px" }}>
                  <div style={{ fontSize: 11, color: "rgba(232,237,248,0.4)", textTransform: "uppercase", letterSpacing: "1px", marginBottom: 6 }}>{rc.disease}</div>
                  <div style={{ fontSize: 18, fontFamily: "'JetBrains Mono',monospace", color: rc.trend === "Worsening" ? "#ff4466" : rc.trend === "Improving" ? "#00e5a0" : "#ffb800", fontWeight: 500, marginBottom: 4 }}>
                    {(rc.current*100).toFixed(0)}%
                  </div>
                  <div style={{ fontSize: 11, color: "rgba(232,237,248,0.4)" }}>
                    was {(rc.previous*100).toFixed(0)}% · {rc.trend === "Worsening" ? "↑" : rc.trend === "Improving" ? "↓" : "→"} {rc.trend}
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* PIPELINE LOG */}
      <div style={card}>
        <div style={secLabel}>⚡ Pipeline Execution Log</div>
        <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 12, lineHeight: 2.2 }}>
          {[
            ...
            ["✓","#00e5a0",`Risk predictor — ${(risks||[]).length} diseases scored`],
            ["✓","#00e5a0",`Anomaly detector — ${(anomalies||[]).length} alerts generated`],
            ["✓","#00e5a0",`Groq LLaMA — clinical summary generated`],
