...
DIAGNOSIS_PHRASES = {
    ...
    "Anemia":         ["iron deficiency anemia", "normocytic anemia", "microcytic anemia", "anemia diagnosed", "severe anemia"],
    "Kidney Disease": ["chronic kidney disease", "ckd stage", "renal failure", "nephritis", "dialysis", "diabetic nephropathy"],
    "Dyslipidemia":   ["dyslipidemia diagnosed", "mixed dyslipidemia", "hyperlipidemia", "lipid disorder"],
    "Hypothyroidism": ["hypothyroidism", "subclinical hypothyroidism", "thyroid disease diagnosed"],
    "Infection":      ["sepsis", "septicemia", "pneumonia diagnosed", "bacterial infection", "viral infection"],
}

DISEASE_MEDS = {
    "Diabetes":       ["insulin glargine", "metformin", "glipizide", "sitagliptin"],
    "Hypertension":   ["amlodipine", "losartan", "telmisartan", "lisinopril", "enalapril", "labetalol"],
...
def extract_lab_numbers(text: str) -> dict:
    ...
    patterns = {
        ...
        "ferritin": [
            r'ferritin[:\s]+(\d+\.?\d*)',
            r'serum\s+ferritin[:\s]+(\d+\.?\d*)',
        ],
        "b12": [
            r'vitamin\s+b12[:\s]+(\d+\.?\d*)',
            r'b12[:\s]+(\d+\.?\d*)',
        ],
    }
...
def check_negation_in_text(disease: str, text: str) -> bool:
    ...
    negation_patterns = [
        f"no {disease_lower}",
        f"no history of {disease_lower}",
        f"no evidence of {disease_lower}",
        f"not {disease_lower}",
        f"rules out {disease_lower}",
        f"non-{disease_lower}",
        f"{disease_lower} ruled out",
        f"{disease_lower}: absent",
        f"{disease_lower}: negative",
...
def predict_risk(entities: dict, text: str) -> list:
    ...
    for disease in DISEASES:
        ...
        # ── STEP 5: ENTITY MATCH ──
        entity_match = any(
            disease.lower() in d or d in disease.lower()
